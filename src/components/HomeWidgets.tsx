...
import { Mail, Clock, Megaphone, Users, Code as CodeIcon } from "lucide-react";
...
export default function HomeWidgets() {
  ...
  useEffect(() => {
    ...
    (supabase as any)
      ...
      .then(({ data }: any) => {
        if (!cancelled && data) setWidgets(data as Widget[]);
      });
  ...
  return (
    <div className="space-y-12">
      {widgets.map((w) => (
        <section key={w.id} className="container">
...
function WidgetRenderer({ widget }: { widget: Widget }) {
  switch (widget.type) {
    ...
    case "top_djs": return <TopDjsWidget config={widget.config} />;
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-2xl">
        ...
        <h2 className="font-display text-3xl md:text-4xl font-bold mb-2">{config.title || "Reste informé des nouveautés"}</h2>
        <p className="text-muted-foreground mb-6">{config.body || "Reçois chaque semaine les meilleures exclus, edits & remixes directement par e-mail."}</p>
        <form onSubmit={submit} className="flex flex-col sm:flex-row gap-3 max-w-md">
          <Input
            type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
            placeholder={config.placeholder || "ton@email.com"}
            className="flex-1"
          />
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-center gap-3 md:gap-6 mt-6 flex-wrap">
        {units.map((u) => (
          <div key={u.label} className="min-w-[72px] md:min-w-[96px] rounded-2xl border border-border bg-card/70 backdrop-blur p-4">
            <div className="font-mono font-bold text-3xl md:text-5xl tabular-nums">{String(u.value).padStart(2, "0")}</div>
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground mt-1">{u.label}</div>
          </div>
        ))}
      </div>
    ...
    <motion.div
      ...
      style={{
        background: config.bg_color
          ? `linear-gradient(135deg, hsl(${config.bg_color}), hsl(${config.bg_color} / 0.7))`
          : undefined,
...
function TopDjsWidget({ config }: { config: any }) {
    ...
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="font-display text-2xl md:text-3xl font-bold flex items-center gap-2">
          <Users className="h-5 w-5 text-primary" /> {config.title || "DJs à la une"}
        </h2>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
        {djs.map((dj) => (
          <Link
            key={dj.id}
            to={`/${dj.kind === "remixer" ? "remixers" : "remixers"}/${dj.slug}`}
