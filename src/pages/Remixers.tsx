...
export default function Remixers() {
  ...
  const pageTitle = isArtistsPage ? "Artistes" : "DJ / Remixer";
  const pageEyebrow = isArtistsPage ? "Voix, groupes & musiciens" : "DJs, remixers & producers";
  const pageDesc = isArtistsPage
    ? "Chanteurs, chanteuses, groupes et musiciens présents sur la plateforme."
    : "Tous les DJs et remixers ayant publié sur la plateforme.";
  const totalLabel = `${artists.length} ${isArtistsPage ? "artiste" : "profil"}${artists.length > 1 ? "s" : ""}`;
  const tracksTotal = artists.reduce((s, r) => s + r.trackCount, 0);
  const trackLabel = `${tracksTotal} ${isArtistsPage ? "track" : "remix"}${tracksTotal > 1 ? "s" : ""}`;

  return (
    <Layout>
      <PageHero
        eyebrow={pageEyebrow}
        title={isArtistsPage ? "Les" : "Les"}
        highlight={pageTitle}
      ...
      <div className="container py-6 space-y-5">
        ...
        <div className="relative">
          ...
          <Input
            placeholder={`Rechercher un ${isArtistsPage ? "artiste" : "DJ ou remixer"}…`}
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="pl-10 pr-10 h-11 bg-card/40 border-border/60"
          />
          {search && (
        ...
        <div className="flex gap-1 overflow-x-auto pb-1 -mx-2 px-2">
          <button
            ...
            className={`shrink-0 px-3 h-8 rounded-md text-[11px] font-bold uppercase tracking-wider border transition-all ${
          ...
          {ALPHABET.map((l) => {
            ...
            return (
              <button
                ...
                disabled={!enabled}
                onClick={() => setLetter(l)}
                className={`shrink-0 w-8 h-8 rounded-md text-[11px] font-bold border transition-all ${
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 rounded-2xl border border-dashed border-border">
            <p className="text-muted-foreground text-sm">
              {isArtistsPage ? "Aucun artiste trouvé." : "Aucun DJ ou remixer trouvé."}
            </p>
            <Button variant="ghost" size="sm" className="mt-3" onClick={() => { setSearch(""); setLetter("all"); setActiveRoles(new Set()); }}>Réinitialiser les filtres</Button>
          </div>
        ) : (
          <motion.div
            initial="hidden" animate="show"
            variants={{ hidden: {}, show: { transition: { staggerChildren: 0.02 } } }}
            className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-x-4 gap-y-6 pt-2"
          >
            {filtered.map((r) => (
              <motion.div key={r.id} variants={{ hidden: { opacity: 0, y: 12 }, show: { opacity: 1, y: 0 } }}>
                <Link to={`/artists/${r.slug}`} className="group block text-center">
                  <div className="relative mx-auto aspect-square w-full rounded-full overflow-hidden bg-gradient-to-br from-secondary to-card ring-1 ring-border group-hover:ring-primary transition-all">
                    {r.photo_url ? (
                      <img src={r.photo_url} alt={r.name} loading="lazy" className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ) : (
                      <div className="absolute inset-0 flex items-center justify-center text-3xl font-display font-black text-muted-foreground">{r.name[0]?.toUpperCase()}</div>
                    )}
                  </div>
                  <p className="mt-2.5 font-display font-bold text-sm group-hover:text-primary transition-colors truncate">{r.name}</p>
